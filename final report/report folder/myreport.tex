...
%TODO forgot to mention the non-verlan part -- to balance them out, I sed a huggingface french dataset
...
Because we are experimenting the performance of Mistral 7B (thereforth Mistral), designing a zero-shot test thus becomes important\;---\;foremostly, we can determine whether Mistral already learnt how to identify Verlan, and to what extent can it identify correctly. Additionally, we can compare the performance of the zero-shot model with the trained models, to see to what extent our dataset augments the performance.

To do so, the report has designed the pipeline as below, with prompt engineering:
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        process/.style={rectangle, rounded corners, draw, fill=gray!5, align=center, minimum width=3.6cm, minimum height=1.05cm},
        line/.style={-Latex}
    ]
        \node[process] (data) {Load zero-shot test datasets};
        \node[process, below=of data] (model) {Load tokenizer \& Mistral model};
        \node[process, below=of model] (loop) {Loop datasets $\rightarrow$ sentences \\ prompt LLM \& collect outputs};
...
%Mistral and ChatGPT -- prompt engineering for baseline and reference
%explain why chosen ChatGPT 5 Codex (High) as the reference -- cuz it is the best model in the world

\subsection{Training Models}
\subsubsection{General Pipeline}
%Input; how to mask the label to prevent data leakage
%Tokenisation
