...
In this exciting chapter, the report introduces the evaluation techniques and the testing datasets we have created. We then analyses the results in general, and in details. After that, we draw the conclusion of the model performance in this chapter.

%Embedding or Encoder; for embedding, need to show embedding space figures 
\subsection{Evaluation Methodology}
\subsubsection{Embedding space}
To evaluate whether the Verlan tokens show a difference position in the embedding space or not, we have decided to visulise the embedding space right after the tokens has been embedded (i.e., before training the encoder). After comparisons between Principal Component Analysis (PCA)\footnote{\url{https://en.wikipedia.org/wiki/Principal_component_analysis}}, t-Distributed Stochastic Neighbor Embedding (t-SNE)\footnote{\url{https://en.wikipedia.org/wiki/T-distributed_stochastic_neighbor_embedding}}, and Uniform Manifold Approximation and Projection (UMAP)\footnote{\url{https://umap-learn.readthedocs.io/en/latest/}}, we have found that UMAP generally has the best visulisation\cite{pearson1901pca,maaten2008tsne,mcinnes2018umap}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/mistral_bert_umap.png}
    \caption{UMAP visualizations of the embedding space showing the distribution of Verlan and standard French tokens\\Class 1: Verlan tokens\\Class 0: normal tokens}
    \label{fig:umap_comparison}
\end{figure}

Based on the result, the report is uncertain to say there is a distinguish of the positions between the Verlan tokens and other tokens in the embedding space. On the other hand, we highly agree that trditional linear classication methods, like logistic regression, should not perform well in this case, as the figure is presenting a non-linear pattern.

\subsubsection{Testing Datasets}
For testing the model performance, the report has created a couple of testing datasets. These datasets are not the sentence dataset, nor the dictionary dataset that we have created for training the models. We have created three datasets as below, inside each of them, there are pairs of sentences contain Verlan, and the same sentences that their Verlan has been normalised:
\begin{enumerate}
  \item Daily Verlan --- 60 entries (30 pairs) of sentence that frequently been used by French speakers.
  \item Invented Verlan --- 50 entries (25 pairs) of sentence that the report has created to mimick tasks to indetify newly invented Verlans.
  \item Slang --- 50 entries (25 pairs) of sentence that contains slang and their normalised pair. All the sentences are labelled as not contains Verlan.
\end{enumerate}

Interestingly, we have created the slang testing dataset, because of the reasons below:
\begin{enumerate}
  \item Slang can also be treated as a type of noise in text, as Verlan does.
  \item The model might be learning to identify slang in general instead of Verlan, the report would like to justify that by creating this dataset.
\end{enumerate}

All the sentences in these datasets have not been appeared in the training dataset.

Readers might argue the quantity of the testing datasets being small. The report believes that this amount is enough for testing purposes.

\subsubsection{Testing Schema}

\paragraph{Zero-shot Models}
For zero-shot models, in particular, GPT 5 Codex (High) and Mistral 7B, we only evaluate them based on the testing datasets instead of the training dataset. As its name, the purpose of the training dataset is for training, thus we believe it is unnecessary to conduct zero-shot experiments on them.

\paragraph{Number of Trials}
As mentioned above, the report runs each model 20 times with 20 different seeds, in order to reduce bias.

\paragraph{Storing the Results}
For easy analysation purpose, the report stores the result of each sentence in each trial, across all four training models and the zero-shot trial of Mistral 7B, in CSV format. For the results of GPT 5 Codex (High), we store them in a spreadsheet.

\paragraph{Analyse Methodology}

\subparagraph{Confusion Matrix}

In the analyses, a 2x2 binary confusion matrix is being used. This is also essential for accuracy and F1 score calculation.
...
To evaluate the accuracy, we use the formula below:
...
The F1 score is the harmonic mean of Precision and Recall, the formula is as below:
...
\begin{figure}[H]
  ...
  \caption{A general comparison of the F1 score and the accuracy of the models.}
