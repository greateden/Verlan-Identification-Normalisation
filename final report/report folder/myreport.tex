...
\node[anchor=north, font=\small, align=center] at ($(h1)+(0,15mm)$) {Linear $4096\!\rightarrow\!4096$\\$\tanh$ activation};

% ===== Output (single logit) =====
\node[linearNeuron,label=right:{$\ell_{\text{detect}}$}] (o1) at ($(C2)+(0,0mm)$) {};
\node[layerlabel] at ($(o1)+(0,18mm)$) {\textbf{Output}};
\node[anchor=north, font=\small, align=center] at ($(o1)+(0,15mm)$) {Linear $4096\!\rightarrow\!1$};

% ===== Dropout annotations =====
\node[draw, dashed, rounded corners=1pt, font=\scriptsize, fill=white, minimum width=17mm, minimum height=42mm]
  (dropBefore) at ($(C0)!0.55!(C1)$) {\shortstack{Dropout\\$p=0.1$}};
\node[draw, dashed, rounded corners=1pt, font=\scriptsize, fill=white, minimum width=17mm, minimum height=42mm]
  (dropBetween) at ($(C1)!0.55!(C2)$) {\shortstack{Dropout\\$p=0.1$}};
