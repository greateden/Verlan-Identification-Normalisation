...
\maketitle
%https://www.cambridge.org/core/journals/journal-of-french-language-studies/article/tracing-the-origins-of-verlan-in-an-early-nineteenth-century-text/C04B0862148D9B44B3C44E675C00CE55
\begin{abstract}
something.
\end{abstract}
...
Since the early 19th century, the French people have started to talk using verlan\;---\footnote{I like using em-dashes.} 
an unusual and creative form of \textit{argot} (slang) that is formed by flipping the syllables around in a word.\footnote{In fact, the word \textit{verlan} is a verlan from the word \textit{l'inver} (the inversion).}\cite{rajabov2025,bach2018} 
Time flies, verlan has become more and more popular, and it is now widely used amongst teens and young people in francophone societies\footnote{Such as France, Belgium, Switzerland, Luxembourg, and Canada.}\cite{evolutionverlan}.

Examples of verlan can be as follows:

\begin{equation}
\text{bite} = \text{bi} + \text{te} 
\;\;\longrightarrow\;\; \text{te} + \text{bi} 
\;\;\longrightarrow\;\; \text{tébi} \; (\text{penis})
\end{equation}

\begin{equation}
\text{shit} = \text{shi} + \text{t} 
\;\;\longrightarrow\;\; \text{t} + \text{shi} 
\;\;\longrightarrow\;\; \text{teuchi}\cite{evolutionverlan}
\end{equation}

\begin{equation}
\text{bonjour} = \text{bon} + \text{jour} 
\;\;\longrightarrow\;\; \text{jour} + \text{bon} 
\;\;\longrightarrow\;\; \text{jourbon} \; (\text{greetings})
\end{equation}

And in real-life conversations, such can be used as in the example sentences below:

\begin{center}
\textit{Le graff géant représente une tebie pixel art.} (The giant graffiti depicts a pixel art penis.) \\[6pt]
\textit{Il a du bon teuchi du bled.} (He's got some good shit from the countryside.) \\[6pt]
\textit{Un p’tit\footnote{Standard spelling: petit.} jourbon et tout le monde sourit.} (A quick hello and everyone smiles.)
\end{center}
...
%TODO continue writing "what questions can be naturally raised", like using ML; pig latin
