...
% \medskip
% \textbf{Français.} Cette étude présente une chaîne de traitement pour détecter et normaliser le verlan à l'aide de grands modèles de langage.
% Nous constituons deux jeux de données complémentaires : un lexique de 1\,086 entrées verlan--français standard et un corpus équilibré de 5\,684 phrases annotées, complétés par trois jeux de test dédiés (Daily Verlan, Invented Verlan, Slang).
% La pipeline d'entraînement fusionne l'encodeur SFR-Embedding-Mistral 7B avec des têtes logistiques ou de type BERT, évaluées sous quatre configurations figées ou entièrement ajustées, chacune répétée sur 20 graines avec quantification 4 bits.
% Le meilleur détecteur entraîné (E2E+BERT) atteint 84.9~\% d'exactitude et 80.5~\% de F1 (\(\pm 4.9\)~pts et \(\pm 5.1\)~pts) sur un jeu de test de 853 phrases, dépassant les variantes linéaires tout en restant en deçà du repère zéro-shot GPT-5 Codex à 91.8~\%.
% Les analyses fines montrent que la généralisation vers du verlan inventé demeure dominée par les modèles zéro-shot, alors que l'apprentissage améliore nettement la spécificité face à d'autres argots.
% Nous publions le code, les jeux de données non annotés et les scripts d'évaluation sous licences ouvertes afin de soutenir des travaux futurs sur la traduction et la détection du verlan.
\end{abstract}
...
\end{tikzpicture}
\caption{\label{fig:pipeline}A visualisation of the objectives.}
\end{figure}

With this purpose, the report contributes two Verlan datasets to the linguistics and AI research communities: one that supports dictionary construction and another geared toward training and evaluating LLMs. The report also evaluates how effectively ML can identify Verlan, with the goal of benefiting future machine-translation systems.

The code and the unannotated, non-peer-reviewed dataset developed for the project are released under open licences and align with open science best practices, using a version-controlled software development platform (GitHub)\footnote{\url{https://github.com/greateden/Verlan-Identification-Normalisation}}. The annotated, peer-reviewed dataset will be published shortly after this report, by the end of 2025.
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \end{adjustwidth}
  \caption{\label{fig:pipeline-overview}A compact view of the four Verlan identification pipelines.\footnotesize \\Exp~A: Frozen Encoder + LogisticRegression Head\\Exp~B: End-to-End Encoder + Linear Head\\Exp~C: Frozen Encoder + BERT-Style Head\\Exp~D: End-to-End Encoder + BERT-Style Head}
\end{figure}
