...
These failures motivate an early attempt at automatic verlan detection in text. Accordingly, this report examines how current machine learning models perform on the detection task. Previous works have tackled detecting slang with ML and translating noisy text more broadly \cite{pei2019slang, michel2018mtnt}.
...
\hline
\textbf{Source} & Publicly available reference (URL/citation if available; archive/screenshot when possible) & Web text with no available reference & OpenAI o3 (template prompt) \\
\hline
\end{tabular}%
}
...
We assess \textit{GPT-5 Codex (High)} in a zero-shot configuration as a reliable, off-the-shelf reference outside of Mistral. We selected GPT5 Codex (High) due to it being the top-recommended reference on the Artificial Analysis Intelligence Index at our time of retrieval (14~Oct~2025; Fig.~\ref{fig:AI_Index}), and its batching capability given our prompt structure. Although we include the leaderboard for context, we draw all conclusions from our test sets.
...
\]
It operates purely on the frozen Mistral embeddings. No BERT Transformer blocks are reused.
...
We therefore focus on two extremes for clarity: \emph{fully frozen} vs.\ \emph{end-to-end fine-tuned}. This choice lets us postpone intermediate variants (e.g., partial layer unfreezing) for future work.
...
The primary difference among the four experiments lies here\;---\;not only in the choice between Logistic Regression and a shallow BERT-inspired multi-layer perceptron (the BERT \texttt{[CLS]} head operating on frozen Mistral embeddings), but also in the implementation framework, namely scikit-learn versus PyTorch.
...
These results imply an intentional architectural trade-off. We decided to use only the BERT \texttt{[CLS]} classifier head, stacked on the frozen Mistral embeddings, because the full-distribution fine-tuning of Transformers would require longer hyperparameter sweeps to avoid overfitting the small, noisy corpus. This choice enhances reproducibility while maintaining the ceiling\;---\;most of the linguistic signal still emanates from the Mistral encoder. Once we have more time to increase the corpus size, we will revisit this with BERT, also with its transformer layers in the pipeline.
...
\addcontentsline{toc}{section}{Appendix \thesection: Participant Information Sheet}
...
\includepdf[
  ...
  pagecommand={%
    ...
    \ifnum\value{appendixpdfpage}=1\relax
      \vspace*{-3cm}%
      \section*{Appendix \thesection: Participant Information Sheet}%
    \fi
  }
]{Info Sheet.pdf}
...
\skipsectioncleartrue
\refstepcounter{section}
\addcontentsline{toc}{section}{Appendix \thesection: Participant Consent Form}
\setcounter{appendixpdfpage}{0}
\includepdf[
  pages=-,
  scale=0.9,
  pagecommand={%
    ...
    \ifnum\value{appendixpdfpage}=1\relax
      \vspace*{-3cm}%
      \section*{Appendix \thesection: Participant Consent Form}%
    \fi
  }
]{Consent Form.pdf}
...
\skipsectioncleartrue
\refstepcounter{section}
\addcontentsline{toc}{section}{Appendix \thesection: Annotated Lexicon --- Eléonore}
\setcounter{appendixpdfpage}{0}
\includepdf[
  pages=-,
  scale=0.8,
  pagecommand={%
    ...
    \ifnum\value{appendixpdfpage}=1\relax
      \vspace*{-3cm}%
      \section*{Appendix \thesection: Annotated Lexicon}%
    \fi
  }
...
\skipsectioncleartrue
\refstepcounter{section}
\addcontentsline{toc}{section}{Appendix \thesection: Annotated Lexicon --- Lua}
\setcounter{appendixpdfpage}{0}
\includepdf[
  pages=-,
  scale=0.8,
