#!/usr/bin/env bash
# Submit zero-shot evaluation jobs for Mistral-7B on Aoraki.
...
JOBID=$(sbatch \
  ...
  --chdir="$REPO_ROOT" \
  --export=ALL \
  scripts/aoraki/mistral_zeroshot.slurm)

echo "Submitted as job $JOBID"
LOGOUT="$LOG_DIR/verlan-mistral-zeroshot-${JOBID}.out"
LOGERR="$LOG_DIR/verlan-mistral-zeroshot-${JOBID}.err"
